...
export up, down, flip, nstate, Bit, Spin, Half
...
Returns the number of different states of given site type.
"""
function nstate end

abstract type BinarySite{T} <: AbstractSite{T} end

Base.:(==)(lhs::BinarySite, rhs::Number) = lhs.val == rhs
Base.:(==)(lhs::Number, rhs::BinarySite) = lhs == rhs.val

Base.convert(TT, x::BinarySite) = convert(TT, x.val)
Base.convert(::Type{SiteType}, x::Number) where {T, SiteType<:BinarySite{T}} =
    x == up(SiteType) ? up(SiteType) :
    x == down(SiteType) ? down(SiteType) :
    throw(InexactError(:convert, SiteType, x))

nstate(::Type{<:BinarySite}) = 2
up(::T) where {T <: BinarySite} = up(T)
down(::T) where {T <: BinarySite} = down(T)
...
struct Half{T <: AbstractFloat} <: BinarySite{T}
    val::T
end
...
flip(x::BinarySite) = x == up(x) ? down(x) : up(x)
